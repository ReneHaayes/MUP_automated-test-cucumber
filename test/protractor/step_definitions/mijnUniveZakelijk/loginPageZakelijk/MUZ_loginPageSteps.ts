...
// Na zakelijke login
Then(/^the title Overzicht appears on screen$/, async () => {
  await genericMethods.clickOnElement(genericElements.cookieClickElement);
  ...
  await genericMethods.waitForElementIsVisible(loginPageElements.loggedInHeaderH1TextZakelijkElement, browser.getPageTimeout);
  await genericMethods.verifyTextInElement(loginPageElements.loggedInHeaderH1TextZakelijkElement, loginPageElements.loggedInHeaderH1ZakelijkText);
  await genericMethods.verifyTextNotEmpty(await genericMethods.getAnalyticsText(analyticsGenericElements.advEventHistory(anaEnumLocation.FIRST, anaEnumObjectName.PAGEVIEW,
    anaEnumSearchObject.KLANT_ONEGINI_ID)));
});
